#script used during the experiment to create network distribution graphs,
...
setwd("C:\\Users\\Youngjun Park\\Desktop\\IoTMU\\Captures4")
...
for (device in devicelist){
  devicename <- read.csv(paste(device, ".csv", sep = "_1")) #run 1
  devicename$Name <- device
  devicename$Time <- devicename$Time - min(devicename$Time)
  tempdevice <- read.csv(paste(device, ".csv", sep = "_2")) #run 2
  tempdevice$Name <- device
  tempdevice$Time <- tempdevice$Time - min(tempdevice$Time) +  720 #add time offset
  devicename <- rbind(devicename, tempdevice)
  tempdevice <- read.csv(paste(device, ".csv", sep = "_3")) #run 3
  tempdevice$Name <- device
  tempdevice$Time <- tempdevice$Time - min(tempdevice$Time) +  1440 #add time offset
  devicename <- rbind(devicename, tempdevice)
  ...
  #extract outliers
  outliers <- boxplot.stats(devicename.hist$counts)$out
  outliers <- outliers[outliers > 0]
  devicename.times <- devicename.hist$breaks[-1][(devicename.hist$counts >= min(outliers))]#remove [-1]
  ...
  if (plots == 1){
    ...
    p <- p + theme_bw() + theme(panel.grid.minor = element_blank())
    p <- p + coord_cartesian(xlim=c(0, 2160), ylim=c(0, 25))
...
if (plots == 1)
{
  ...
  plot(p)
  ggsave(file=paste("pca_scree", ".png", sep = trial), height = 5, width = 7) 
}
...
#################
if (plots == 1){
  shapes = c(17, 17, 16, 16, 17, 17, 16, 17, 17, 16, 16)
  colors <- c("#E69F00", "#E69F00", "#E69F00","#E69F00", "#56B4E9", "#56B4E9", "#56B4E9", "#999999", "#999999", "#999999", "#999999")
  graphics.off()
  png(filename = paste("PCA", ".png", sep = trial), width = 8, height = 5.9, units = "in", res = 300)
