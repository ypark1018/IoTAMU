#! /user/bin/env python
#
# Decrypt data received from the remote interface
#
...
def main():
    ...
    while 1:
        ...
        payload = 0
        try:
            raw_data,addr=t.recvfrom(65536) #data received from eth0
            ...
            if payload != 0:
                if parse.ipv4(src_ip) != "192.168.1.17":
                    continue
                print("Message received: " + str(payload))
                new_data = enc_data(payload, encryptor)
                print("Message encrypted: " + str(new_data))

                #MAC address of the IoT
                dst_mac = b"\x80\x2a\xa8\x9e\x45\x5a"
                ...
                if ip_proto == 17: #new headers
                    ...
                    s.send(dst_mac+raw_data[6:i_offset]+i_header+t_header+new_data)
...
def initialize_enc():
    ...
    key = b"11111111111111111111111111111111"#same key used in RI and IoTAMU
